...
def get_full_data(term: str, location: str, yelp_key: str, file_name: str) -> pd.DataFrame:
    """
    ...
    for num in range(num_pages):
        try:
            ...
            with open(file_name, 'a') as file:
                parsed_results.to_csv(file, mode='a', index=False, header=(num == 0), encoding="latin1")
...
def clean_data(raw_data_path, cleaned_data_path, top_num_aliases = 2):
    '''
    - Requires data from either an existing dataframe or an existing .csv file
    - Takes raw business data from the Yelp API and filters for the top two aliases.
    '''

    # Read in businesses
    df1 = pd.read_csv(raw_data_path, encoding = 'latin1')

    alias_index = df1['alias'].value_counts()[:top_num_aliases].index
    print(alias_index)
